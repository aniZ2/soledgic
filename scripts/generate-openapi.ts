...
const SCHEMAS: Record<string, object> = {
  ...
  ReverseTransactionRequest: {
    ...
    properties: {
      ...
      idempotency_key: { type: 'string', description: 'Unique key to prevent duplicate reversals' },
      metadata: { type: 'object', additionalProperties: true },
    },
  ...
  TreasuryRefundRequest: {
    ...
    properties: {
      ...
      processor_payment_id: { type: 'string', description: 'Processor payment ID to refund (required when mode is processor_refund)' },
      metadata: { type: 'object', additionalProperties: true },
    },
  ...
  ReverseTransactionResponse: {
    allOf: [
      { $ref: '#/components/schemas/SuccessEnvelope' },
      {
        type: 'object',
        properties: {
          void_type: { type: 'string', enum: ['soft_delete', 'reversing_entry'], description: 'Whether the transaction was soft-deleted or reversed via entries' },
          reversal_id: { type: ['string', 'null'], format: 'uuid', description: 'ID of the reversal transaction (null for some soft deletes)' },
          original_transaction_id: { type: 'string', format: 'uuid', description: 'ID of the original transaction that was reversed' },
          reversed_amount: { type: ['number', 'null'], description: 'Reversed amount in major currency units (present for reversing entries)' },
          is_partial: { type: ['boolean', 'null'], description: 'Whether this was a partial reversal' },
          voided_at: { type: ['string', 'null'], format: 'date-time', description: 'Timestamp when the transaction was voided (soft delete path)' },
          reversed_at: { type: ['string', 'null'], format: 'date-time', description: 'Timestamp when the reversing entries were created' },
          warning: { type: ['string', 'null'], description: 'Warning message (e.g. when reversing a reconciled transaction)' },
        },
      },
    ],
  },
