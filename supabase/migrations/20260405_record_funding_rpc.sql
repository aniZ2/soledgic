-- record_funding_atomic: Records external money entering the system.
-- Stripe → buyer wallet. NO creator, NO platform split, NO Soledgic fee.
...
BEGIN
  ...
  v_net_cents := p_amount_cents - p_processing_fee_cents;
  ...
  -- DR stripe_clearing (money arrived from Stripe)
  ...
  -- CR buyer_wallet (net after Stripe fee)
  INSERT INTO public.entries (transaction_id, account_id, entry_type, amount)
  VALUES (v_tx_id, v_buyer_wallet_id, 'credit', v_net_cents / 100.0);

  -- CR processing_fees (Stripe's cut)
  IF p_processing_fee_cents > 0 AND v_fee_account_id IS NOT NULL THEN
    INSERT INTO public.entries (transaction_id, account_id, entry_type, amount)
    VALUES (v_tx_id, v_fee_account_id, 'credit', p_processing_fee_cents / 100.0);
  END IF;
