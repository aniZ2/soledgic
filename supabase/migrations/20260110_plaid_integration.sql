-- Soledgic: Plaid Bank Feed Integration
-- Auto-sync bank transactions via Plaid
...
CREATE TABLE IF NOT EXISTS plaid_connections (
  ...
  -- Linked accounts
  accounts JSONB DEFAULT '[]'::jsonb, -- Array of Plaid account objects
...
CREATE INDEX idx_plaid_connections_ledger ON plaid_connections(ledger_id);
CREATE INDEX idx_plaid_connections_status ON plaid_connections(status) WHERE status = 'active';
...
CREATE TABLE IF NOT EXISTS plaid_transactions (
  ...
  -- Transaction details
  amount NUMERIC(15,2) NOT NULL, -- Positive = outflow, Negative = inflow (Plaid convention)
  date DATE NOT NULL,
  name TEXT NOT NULL,
  merchant_name TEXT,
  category TEXT[],
  ...
  UNIQUE(ledger_id, plaid_transaction_id)
);

CREATE INDEX idx_plaid_txns_ledger ON plaid_transactions(ledger_id);
CREATE INDEX idx_plaid_txns_unmatched ON plaid_transactions(ledger_id, match_status) WHERE match_status = 'unmatched';
CREATE INDEX idx_plaid_txns_date ON plaid_transactions(ledger_id, date);
...
-- Auto-match a single Plaid transaction
CREATE OR REPLACE FUNCTION auto_match_plaid_transaction(p_plaid_txn_id UUID)
...
CREATE OR REPLACE FUNCTION check_auto_match_conditions(p_txn plaid_transactions, p_conditions JSONB)
RETURNS BOOLEAN AS $$
BEGIN
  -- Merchant contains
...
-- Via API key
CREATE POLICY "Plaid connections via API key"
...
CREATE POLICY "Plaid transactions via API key"
...
COMMENT ON TABLE plaid_connections IS 'Plaid Link connections for bank account syncing';
...
COMMENT ON FUNCTION auto_match_plaid_transaction IS 'Attempt to auto-match a Plaid transaction using rules';
